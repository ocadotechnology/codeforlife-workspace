...
function comment_on_issue() {
  ...
  local issue_repo="$2"
  local comment_path="$3"
  local substitutions="${@:4}"

  local issue_comment_body="$(
    make_comment "issue/inactive/$comment_path" "$substitutions"
  )"
...
function process_repo() {
  ...
  local issues=$(
    gh issue list \
      --repo=$repo \
      --limit=10000 \
      --search="is:open has:assignee $cfl_bot_ignore_label_filter" \
  ...
  echo "$issues" | jq -c '.[]' | while read -r issue; do
    ...
    while read -r issue_assignee; do
      ...
      if is_ge_start_and_lt_end_days_old \
        "$issue_assignee_last_active_at" \
        15 14; then
        comment_on_issue "$issue_number" "$repo" "warning.md" \
          "assignee=@$issue_assignee_login"
      elif is_ge_start_and_lt_end_days_old \
        "$issue_assignee_last_active_at" \
        0 21; then
        comment_on_issue "$issue_number" "$repo" "unassigned.md" \
          "assignee=@$issue_assignee_login"
